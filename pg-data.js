...
module.exports = function(dbconf) {
	...
	var conn = "postgres://" + dbconf.user + ":" + dbconf.password + "@" + dbconf.host + "/" + dbconf.database;
	...
	function loadNewest(cb) {
		...
		pg.connect(conn, function(err, client, done) {
			...
			if (err) {
				...
				var sql = 'SELECT id, pub_date, to_char(pub_date, \'Day, DD Month YYYY\') as pd, data FROM comic.comic_data ' +
				'WHERE pub_date = (SELECT MAX(pub_date) FROM comic.comic_data WHERE DATE(pub_date) <= NOW())';
				
				console.log("loading newest from pg");
	...
	return {
			...
			if (isNaN(id)) {
				...
				pg.connect(conn, function(err, client, done) {
					...
					if (err) {
						...
						var sql = 'SELECT pub_date, to_char(pub_date, \'Day, DD Month YYYY\') as pd, data ' +
						'FROM comic.comic_data WHERE id = $1';
						
						console.log("loading id " + id + " from pg");
		...
		storeData: function(data, idOrCb, cb) {
			...
			var params;
			if (id) {
//				sql = 'INSERT INTO comic.comic_data (id, pub_date, data) VALUES ($1, $2, $3) ' + 
//					'ON DUPLICATE KEY UPDATE pub_date = VALUES(pub_date), data = VALUES(data)';
				sql = 'UPDATE comic.comic_data SET pub_date = $1, data = $2 WHERE id = $3';
				params = [data.pubDate, json, id];
			} else {
				sql = 'INSERT INTO comic.comic_data (pub_date, data) VALUES ($1, $2) RETURNING id';
